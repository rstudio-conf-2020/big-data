...
1. Use `collect()` bring back the aggregated results into a "pass-through" variable called `by_quarter`
    ```{r, eval = eval_viz}
    by_quarter <- transactions %>%
      count(order_date_quarter) %>%
      collect()
    ```

1. Preview the `by_quarter` variable
    ```{r, eval = eval_viz}
    by_quarter
    ```
...
1. Plot results using `ggplot2`
    ```{r, eval = eval_viz}
    ggplot(by_quarter) +
      geom_col(aes(order_date_quarter, n)) 
    ```
...
1. With the same technique, create a plot of the average `price` by quarter
    ```{r, eval = eval_viz}
    transactions %>%
        group_by(order_date_quarter) %>%
        summarise(avg_sales = mean(price)) %>%
        collect() %>%
        ggplot() +        # < Don't forget to switch to `+`
        geom_col(aes(order_date_quarter, avg_sales)) 
    ```
