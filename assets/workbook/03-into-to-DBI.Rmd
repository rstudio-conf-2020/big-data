...
# Introduction to `DBI` and `RSQlite`
...
````markdown
    ...
    ```
````
```{sql, connection=con, eval = eval_dbi, echo = FALSE}
    select * from mtcars
```
...
````markdown
    ...
    ```
````
```{sql, connection=con, eval = eval_dbi, echo = FALSE, max.print = 5}
    select * from mtcars
```
