...
4. Load the `dplyr` and `dtplyr` libraries
    ```{r, eval = eval_dtplyr}
    library(dplyr)
    library(dtplyr)
    ```

5. Use the `lazy_dt()` to "wrap" the `transactions` variable, into a new variable called `dt_transactions`
    ```{r, eval = eval_dtplyr}
    dt_transactions <- lazy_dt(transactions)
    ```


6. View the `dt_transactions` variable's structure with `glimpse()`
    ```{r, eval = eval_dtplyr}
...
3. Copy the aggregation code above, **then collect it into a `tibble`**, and then use `pivot_wider()` to make the `date_day` the column headers.
    ```{r, eval = eval_dtplyr}
    dt_transactions %>%
      group_by(date_month, date_day) %>% 
      summarise(total_sales = sum(price)) %>%
...
2. Use `lazy_dt()` with the `immutable` argument set to `FALSE` to avoid the copy
    ```{r, eval = eval_dtplyr}
